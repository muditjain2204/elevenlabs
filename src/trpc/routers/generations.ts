...
export const generationsRouter = createTRPCRouter({
    getById: orgProcedure 
        ...
        .query(async ({ input, ctx })  =>{
            //we use prisma to find unique generations 
    ...
    create: orgProcedure
      ...
      try {
        const generation = await prisma.generation.create({
          data: {
            ...
            topP: input.topP,
            topK: input.topK,
            repetationPenalty: input.repetitionPenalty,
          },
          select: {
            id: true,
          },
      ...
      if (env.POLAR_ACCESS_TOKEN && env.POLAR_METER_TTS_GENERATION && env.POLAR_METER_TTS_PROPERTY) {
        polar.events
          .ingest({
            events: [
              {
